...
const CustomizationDrawer = dynamic(
  () =>
    import('@/components/domain/CustomizationDrawer').then((mod) => ({
      default: mod.CustomizationDrawer,
    })),
  { ssr: false },
);
...
export default function DinerPageClient() {
  ...
  const { userName, isHydrated } = useIdentity();
  ...
  const getItemQuantity = useCallback(
    (menuItemId: string) => {
      ...
      return (
        session?.cart.filter(
          (i) => i.menuItemId === menuItemId && !i.isCustomized && i.orderedByName === userName,
        ).length || 0
      );
    },
  ...
  const handleUpdateQuantity = useCallback(
    (menuItemId: string, newQuantity: number) => {
      ...
      if (diff > 0) {
        ...
        // Remove instances
        const instance = session?.cart.find(
          (i) => i.menuItemId === menuItemId && !i.isCustomized && i.orderedByName === userName,
        );
  ...
  const getIdenticalItemCount = useCallback(
    (cartItem: CartItem) => {
      return (
        session?.cart.filter(
          (i) =>
  ...
  const handleAddToCartFromDrawer = useCallback(
    (customizedItem: Partial<CartItem>) => {
  ...
  const showMenu = isHydrated && !!userName && isReady;
            ...
            {categories.map((category) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  <h2 className="text-xl font-serif font-bold text-foreground/90 tracking-tight">
                    {category}
                  </h2>
